...
import '../App.css'
import ShoesAPI from '../services/ShoesAPI.jsx';
import shoeData from '../data/shoe.js';
import shoeUtils from '../utils/ItemUtils.js';
...
const EditShoe = () => {
   const [shoe, setShoe] = useState(null);
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        if (!shoe.name) {
            alert('Please enter a name for the shoe.');
            return;
        }
        ...
        console.log(shoe);
        await ShoesAPI.updateShoe(shoe.id, cleanedShoe);
    ...
    return (
        <main style={{ justifyContent: 'center', gap: '2rem', backgroundColor:'rgba(0,0,0,0.3)' }}>
            <div className="item-display">
                <h2>{shoe.name || 'Update This Shoe'}</h2>
